...
--     warn("❌ This script must only be executed from the offical VicatHub loader")
--         return
--     end
...
--     return
-- end
-- local allowedTimeWindow = 180  -- seconds
...
local Update = (loadstring(Game:HttpGet("https://raw.githubusercontent.com/Meo73preb/MeoYeuEm/refs/heads/main/Load_ui.lua")))();
...
(getgenv()).Load = function()
	if readfile and writefile and isfile and isfolder then
		if not isfolder("VicatHub") then
			makefolder("VicatHub");
		end;
		if not isfolder("VicatHub/BF/") then
			makefolder("VicatHub/BF/");
		end;
		if not isfile(("VicatHub/BF/" .. game.Players.LocalPlayer.Name .. ".json")) then
			writefile("VicatHub/BF/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(_G.Settings));
		else
			local Decode = (game:GetService("HttpService")):JSONDecode(readfile("VicatHub/BF/" .. game.Players.LocalPlayer.Name .. ".json"));
	...
	if Update:SaveSettings() then
		if readfile and writefile and isfile and isfolder then
			if not isfile(("VicatHub/BF/" .. game.Players.LocalPlayer.Name .. ".json")) then
				...
				writefile("VicatHub/BF/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(Array));
			end;
		else
			return warn("Status : Undetected Executor");
		end;
...
spawn(function()
	...
	wait(1);
	((require(game.ReplicatedStorage.Notification)).new("<Color=Yellow>[ Enjoy for VicatHub ]<Color=/>")):Display();
end);
function CheckQuest()
...
spawn(function()
	while wait(0.2) do
		if _G.Settings.Farm["Selected Bone Farm Mode"] == "Quest" and _G.Settings.Farm["Auto Farm Bone"] and World3 then
			pcall(function()
				local QuestData = CheckBoneQuest()
				
				-- Nếu level không đủ, chuyển về No Quest
				if not QuestData then
					_G.Settings.Farm["Selected Bone Farm Mode"] = "No Quest"
					return
				end
				...
				if (game:GetService("Players")).LocalPlayer.PlayerGui.Main.Quest.Visible == false then
					topos(QuestData.CFrameQuest);
					if (QuestData.CFrameQuest.Position - (game:GetService("Players")).LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 3 then
				...
				elseif (game:GetService("Players")).LocalPlayer.PlayerGui.Main.Quest.Visible == true then
					...
					for i, v in pairs((game:GetService("Workspace")).Enemies:GetChildren()) do
						if v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then
							-- Nếu là Posessed Mummy, tìm quái gần nhất
							if QuestData.Mon == "Posessed Mummy" then
								if v.Name == "Reborn Skeleton" or v.Name == "Living Zombie" or v.Name == "Demonic Soul" or v.Name == "Posessed Mummy" then
					...
					-- Đánh quái đã chọn
					if vBone then
						if string.find((game:GetService("Players")).LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, QuestData.NameMon) then
							repeat
								(game:GetService("RunService")).Heartbeat:wait();
								EquipWeapon(_G.Settings.Main["Selected Weapon"]);
								AutoHaki();
								PosMon = vBone.HumanoidRootPart.CFrame;
								MonFarm = vBone.Name;
								topos(vBone.HumanoidRootPart.CFrame * Pos);
								vBone.HumanoidRootPart.CanCollide = false;
								vBone.Humanoid.WalkSpeed = 0;
								vBone.Head.CanCollide = false;
								Attack();
							until not _G.Settings.Farm["Auto Farm Bone"] or vBone.Humanoid.Health <= 0 or (not vBone.Parent) or (game:GetService("Players")).LocalPlayer.PlayerGui.Main.Quest.Visible == false;
						end
					else
						-- Không tìm thấy quái, di chuyển đến vị trí spawn
						topos(QuestData.CFrameMon);
					end;
				end;
			end);
		end;
...
spawn(function()
	while wait() do
		...
		elseif _G.Settings.Setting["Fast Attack Mode"] == "Risk" then
...
SettingsTab:Toggle("Kill Aura (risk)", _G.Settings.Setting["Kill Aura"], "Kill Mob", function(value)
    _G.Settings.Setting["Kill Aura"] = value
    getgenv().SaveSetting()
end)
...
spawn(function()
    while task.wait(0.2) do
        pcall(function()
            if _G.Settings.Setting["Kill Aura"] then
                            ...
                            if distance <= 220 then
                                repeat
                                    ...
                                    if sethiddenproperty then
                                        sethiddenproperty(LocalPlayer, "SimulationRadius", 220)
                                    end
                                until not _G.Settings.Setting["Kill Aura"] or not enemy.Parent or enemy.Humanoid.Health <= 0
                            end
                        end
                    end
                end
...
function AttackModule:AttackNearest()
	...
	for _, Enemy in enemyFolder:GetChildren() do
		if not args[1] and Enemy:FindFirstChild("HumanoidRootPart", true) and client:DistanceFromCharacter(Enemy.HumanoidRootPart.Position) < 55 then
			args[1] = Enemy:FindFirstChild("HumanoidRootPart");
		elseif Enemy:FindFirstChild("HumanoidRootPart", true) and client:DistanceFromCharacter(Enemy.HumanoidRootPart.Position) < 55 then
			table.insert(args[2], {
				[1] = Enemy,
...
-- THÊM CODE MỚI: Fruit M1 Attack
AttackModule.FruitM1Data = {
	Combo = 0,
	LastAttack = 0,
	MaxCombo = 4,
	ComboResetTime = 0.3
...
function AttackModule:GetFruitCombo()
	local now = tick();
	if (now - self.FruitM1Data.LastAttack) > self.FruitM1Data.ComboResetTime then
		self.FruitM1Data.Combo = 0;
	end;
	self.FruitM1Data.Combo = self.FruitM1Data.Combo + 1;
	if self.FruitM1Data.Combo > self.FruitM1Data.MaxCombo then
		self.FruitM1Data.Combo = 1;
	end;
	self.FruitM1Data.LastAttack = now;
	return self.FruitM1Data.Combo;
end;

function AttackModule:UseFruitM1()
	...
	-- Tìm MonPos (vị trí monster) trong 50 studs
	local MonPos = nil;
	...
	for _, enemy in enemyFolder:GetChildren() do
		...
		if enemyHRP and enemyHum and enemyHum.Health > 0 then
			...
			if dist <= 50 then
				MonPos = enemyHRP.Position;
				break;
			end;
		end;
	end;
	
	if not MonPos then return false end;
	
	local combo = self:GetFruitCombo();
	
	pcall(function()
		leftClickRemote:FireServer(MonPos, combo);
	end);
...
function Attack()
	if not _G.Settings.Main["Auto Farm Fruit Mastery"] or (not _G.Settings.Main["Auto Farm Gun Mastery"]) then
		...
		elseif _G.Settings.Main["Select Weapon"] == "Blox Fruit" then
			local success = AttackModule:UseFruitM1();
			if not success then
				AttackModule:BladeHits();
			end;
		else
		...
		pcall(function()
			while wait(0.2) do
				if game.Workspace._WorldOrigin.Locations:FindFirstChild("Kitsune Island") then
					Kitsunecheck:Set("✅️ Kitsune Island Spawning");
				...
				if game.Workspace._WorldOrigin.Locations:FindFirstChild("Frozen Dimension") then
					Frozencheck:Set("✅️ Frozen Dimension Spawning");
					_G.FrozenSpawn = true;
				else
					Frozencheck:Set("❌️ Frozen Dimension Not Spawn");
					_G.FrozenSpawn = false;
				end;
				if game.Workspace._WorldOrigin.Locations:FindFirstChild("Prehistoric Island") then
					Prehistoriccheck:Set("✅️ Prehistoric Island Spawning");
					_G.PrehistoricSpawn = true;
				else
					Prehistoriccheck:Set("❌️ Prehistoric Island Not Spawn");
...
spawn(function()
	(game:GetService("RunService")).RenderStepped:Connect(function()
		...
		if sethiddenproperty then
			sethiddenproperty(game.Players.LocalPlayer, "SimulationRadius", 350);
		end;
	end);
end);
...
if World3 then
	...
	RaceV4Tab:Toggle("Auto Trial Race V4", _G.Settings.Race["Auto Trial"], false, function(value)
	...
	while task.wait() do
		pcall(function()
			...
			elseif (game:GetService("Lighting")).Sky.MoonTextureId == "http://www.roblox.com/asset/?id=9709150401" then
				FM:Set("error icon  Full Moon 25%");
...
spawn(function()
	pcall(function()
		while wait(0.2) do
			for i, v in pairs(game.Workspace:GetChildren()) do
				if string.find(v.Name, "Fruit") then
					FindFruit:Set("✅️ Find " .. v.Name);
				else
					FindFruit:Set("❌️ Nothing");
				end;
			end;
		end;
	end);
...
spawn(function()
	while wait(0.2) do
		pcall(function()
			local response = (((game:GetService("ReplicatedStorage")):WaitForChild("Remotes")):WaitForChild("CommF_")):InvokeServer("ColorsDealer", "1");
			if response then
				HakiDealer:Set("✅️ Master Of Auras Spawning");
			else
				HakiDealer:Set("❌️ Master Of Auras Not Spawn");
			end;
		end);
	end;
end);
