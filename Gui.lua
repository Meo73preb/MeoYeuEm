...
function TCH:Window(cfg)
	...
	-- Quả bóng bật/tắt (Floating Button)
	...
	function wnd:Tab(name)
		...
		if not wnd.CurrentTab then tBtn.MouseButton1Click:Fire() end

		function tab:Label(txt)
			C("TextLabel", {BackgroundTransparency = 1, Size = UDim2.new(1, -16, 0, 20), Text = txt, TextColor3 = wnd.ThemeCol, Font = Enum.Font.GothamBold, TextSize = 14, TextXAlignment = Enum.TextXAlignment.Left, Parent = page})
		end
		...
		function tab:Dropdown(txt, options, multi, cb)
			...
			return {
				Refresh = function(newOpts) options = newOpts; selected = multi and {} or nil; dBtn.Val.Text = getTxt(); render() end
			}
		end
